...
Você deseja CSS personalizado no projeto?
  ...
  1. Sim — quero efeitos visuais avançados
  2. Não — seguimos sem CSS customizado
```

**Se Sim:** Pergunte antes de prosseguir para a confirmação de arquitetura:
- Quais elementos devem receber efeitos? (headlines, botões CTA, cards, seções, scrollbar)
- Qual estilo visual do projeto? (clean/minimalista, glassmorphism, gradientes vibrantes, dark, premium)
- Tem referência visual? (link, print, código CSS existente — pode enviar o código que replico e adapto)
- Se nenhuma referência for enviada: a skill escolhe os efeitos mais adequados ao tipo de página e gera o CSS automaticamente com base no catálogo de efeitos

**Se Não:** Registre `CSS_MODULE: desativado` e siga para a Confirmação de Arquitetura.

---

### Passo 6 — Confirmação de Arquitetura
...
4. **Indique os módulos ativos** (CSS Expert se ativado)
...
- [ ] Todos os efeitos `.ea-*` gerados com cores adaptadas à marca do projeto
- [ ] Efeitos que precisam de JS: instrução de widget HTML incluída no css-guide.html
- [ ] Scrollbar personalizada sugerida e incluída
- [ ] css-guide.html gerado na raiz do projeto com instruções para todos os efeitos
- [ ] Código CSS comentado por bloco
...
A skill usa três fontes para gerar os efeitos, nesta ordem de prioridade:

1. **Código enviado pelo usuário** — se o usuário enviar CSS de referência ou de um site, a skill analisa, replica e adapta com as cores da marca do projeto
2. **Nome do efeito** — se o usuário indicar o nome do efeito desejado (ex: `hover-lift`, `glass-effect`, `text-shimmer`), a skill gera o CSS correspondente
3. **Escolha automática** — se o usuário não enviar nada, a skill escolhe os efeitos mais adequados ao tipo de página:

| Tipo de Página | Efeitos Automáticos |
|---------------|---------------------|
...
| Página SaaS | glass-effect, gradient-animated-text, gradient-border, hover-glow-soft, shine-border |
| Site de Agência | hover-tilt-3d, aurora-text, gradient-shift, neon-border, text-shimmer |
| Negócio Local | hover-lift, soft-shadow-card, hover-shadow-deep, glow-pulse |
...
| `hover-grow` | Aumenta levemente com transição suave |
...
| `hover-tilt-3d` | Inclinação 3D baseada no cursor *(requer snippet JS no widget HTML)* |
| `hover-skew-soft` | Inclinação lateral estilizada |
...
| `noise-overlay` | Textura/grão fino estilo premium |
...
| Nome | Descrição |
|------|-----------|
| `ripple-click` | Efeito de onda ao clicar (CSS puro com limitações) |
| `magnetic-button` | Botão reage ao cursor magneticamente *(requer snippet JS)* |
| `cursor-reactive` | Elemento responde ao movimento do mouse *(requer snippet JS)* |
| `press-depress` | Efeito realista de botão pressionado |
| `active-scale` | Escala ao clicar para feedback visual |
| `smooth-transform` | Transições extremamente suaves |
...
| `letter-spacing-breathe` | Animação sutil no espaçamento das letras |
| `text-reveal-mask` | Texto revelado com máscara animada (CSS puro) |
...
| `metallic-text` | Acabamento metálico |
| `chrome-text` | Efeito cromado/reflexivo |
| `holographic-text` | Efeito holográfico futurista |
...
| Nome | Descrição |
|------|-----------|
| `custom-scrollbar` | Scrollbar personalizada com cores da marca — **sempre sugerida em todo projeto** |
...
- Adapta **todas** as cores dos efeitos às cores da marca definidas no design system do projeto
- Efeitos que precisam de JS: gerar o CSS normalmente + instrução de adicionar widget HTML com o snippet JS
- Máximo 2 efeitos visuais por elemento
- A scrollbar customizada (`.ea-scrollbar`) é sugerida automaticamente em todo projeto
- CSS gerado comentado por bloco para facilitar manutenção

---

## Entrega do CSS Guide

Ao finalizar a construção da página, se o módulo CSS Expert estiver ativo, a skill gera automaticamente um arquivo `css-guide.html` na raiz do projeto com as instruções completas de aplicação manual de cada efeito CSS.

### Estrutura do css-guide.html

O arquivo deve ser um HTML com visual limpo e organizado, contendo uma seção para cada efeito CSS gerado no projeto. Cada seção deve ter:

1. Nome e descrição do efeito
2. Em qual elemento aplicar
3. Bloco de código CSS para copiar
4. Passo a passo de aplicação no Elementor:
   - **Passo 1**: Ir em Configurações da Página → Avançado → CSS Personalizado → colar o código
   - **Passo 2**: Selecionar o widget alvo no Elementor
   - **Passo 3**: Ir em Avançado → Classes CSS → colar o nome da classe `.ea-[nome]`
5. Se o efeito precisar de JS: instrução adicional de adicionar widget HTML com o snippet
...
- Blocos de código com fundo destacado (`#1e293b`) e botão "Copiar"
- Passos numerados com ícones visuais
- Header com nome do projeto e data de geração

### Quando Gerar o css-guide.html

Gerar imediatamente após finalizar a construção da página no Elementor, antes de declarar o projeto concluído. Salvar na raiz do projeto VS Code — não dentro do WordPress.
